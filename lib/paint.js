...
(function() {
    ...
    CORE.toggleClass = function toggleClass(node, class_name, callback, toggle_state) {
        ...
        if(node.length >= 0){
            ...
            var i = 0;
            while(i < node.length) CORE.toggleClass(node[i++], class_name, callback && i === node.length - 1 ? callback : void 0);
            return;
        }
    ...
    CORE.setHTML = function setHTML(_node, _html, _async){
        ...
        if(has_callback) /** @type {Function} */ (async).call(node);
    };
