...
APP.VIEWPORT = (function(){
    ...
    var forceOrientation = function(){
        ...
        if(DEBUG) CORE.console.log('Setup Orientation: ' + CONFIG.FORCE_ORIENTATION);
    ...
    */
    ...
    var register_viewport_update = function(){
        ...
        if(resizeTime) CORE.clear(resizeTime);
    ...
    function setupViewportAndZoom(){
        ...
        if(DEBUG) CORE.console.log('Setup Viewport');
        ...
        if((newHeight !== window_height) || (newWidth !== window_width)) {
            ...
            CORE.setStyle(window.document.body, {
                ...
                'width': newWidth + 'px',
                'height': newHeight + 'px',
                'transform': 'scale(' + zoom + ') translateX(' + ((window_width - newWidth) / zoom / 2) + 'px) translateY(' + ((window_height - newHeight) / zoom / 2) + 'px)'
            });
        }
        else{
        ...
        if(DEBUG) CORE.console.log('Resolution (W/H): ' + window_width + ' x ' + window_height + ' --> ' + newWidth + ' x ' + newHeight + ' (Zoom: ' + zoom + ')');
    }

    function lock_orientation(lock_orientation){

        if(DEBUG) CORE.console.log("Lock Orientation: " + lock_orientation);
