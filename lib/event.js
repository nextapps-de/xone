...
(function() {
    ...
    CORE.handleEvent = function(event, elem, fn, preventDefault, stopBubble){
        ...
        if(name) if(DEBUG) CORE.console.log('Event: ' + name);
                        ...
                        for(var x = 0; x < bubble_stack.length; x++) {
                            ...
                            if(tag_name) {
                                ...
                                if(bubble_src){
                                    ...
                                    if(DEBUG) CORE.console.log('Event: ' + event_type, target);
                                }
    ...
    function register_dispatcher(elem, event_type){
        ...
        if(DEBUG) CORE.console.log('Register Event Dispatcher: ' + event_type);

        document.body.addEventListener(

            event_type,
            event_dispatcher,
            event_type === 'touchmove' ? APP.CONFIG.EVENT_OPTIONS : false
        );
    }
    ...
    CORE.on = function on(elem, query, event, _fn, preventDefault, stopBubble, key) {
        ...
        if(elem.length >= 0){
            ...
            var i = 0;
            while(i < elem.length) CORE.on(elem[i++], query, event, _fn, preventDefault, stopBubble, key);
            return fn;
        }
