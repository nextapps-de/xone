        ...
        function(query_or_nodes, properties, duration, easing, callback){
            ...
            for(var i = 0; i < query_or_nodes.length; i++){
                ...
                (function(element, i, properties, duration, callback){
                    ...
                    var old_style_value = CORE.getStyle(element, 'willChange');
                    var new_style_value = old_style_value + ', ' + CORE.getKeys(properties).join(', ');

                    if(!old_style_value || !CORE.contains(old_style_value, 'contents')){

                        properties['willChange'] = new_style_value;
                    }
